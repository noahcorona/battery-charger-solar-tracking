...
static volatile uint32_t msTicks_A = 0;
static volatile uint32_t msTicks_B = 0;
static volatile delayTicks = 0;
extern volatile uint32_t controlLoopMs;
...
void SysTick_Init(void) {
	...
	// MSI configuration -> f_clk = 8 MHz
	// clksource config gives f_clk = f_clk / 8
	// T = 1ms = 10^-3 sec
	// load = (T * f / 8) - 1
	//		  = (10^-3 sec) * (8*10^6) / 8 - 1 = 999
	SysTick->LOAD = 9999;
	...
	NVIC_SetPriority(SysTick_IRQn, 0); // Set Priority to 0 (highest)
...
void delay(uint32_t T) {
	...
	uint32_t endMs = (delayTicks + T);
	// wait T ms
	while(delayTicks < endMs);
